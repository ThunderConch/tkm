...
function createEnvironment() {
  ...
  const roomService = new RoomService({
    ...
    partyService,
    now: clock.now,
    roomCodeGenerator: () => ['A7KQ2M', 'B8TR4N'][roomCodeIndex++] ?? 'Z9YX8W',
  ...
  const registerParty = (playerId: string, generation: PvpGeneration = 'gen1') => {
    ...
    const members: OnlinePartyMemberInput[] = [
      ...
      makeMember(3, '025', 45, ['guest-fast', 'host-fast', 'guest-chip', 'host-chip']),
      makeMember(4, '039', 40, ['guest-chip', 'host-chip', 'guest-fast', 'host-fast']),
      makeMember(5, '052', 35, ['host-fast', 'guest-fast', 'host-chip', 'guest-chip']),
      makeMember(6, playerId === 'host-user' ? '004' : '001', 30, ['guest-chip', 'guest-fast', 'host-chip', 'host-fast']),
    ];
...
describe('PvP timeout policy', () => {
  beforeEach(() => {
    initLocale('ko');
  });

  it('액션 타임아웃이 나면 서버가 기본 유효 명령을 자동 제출해 턴을 진행한다', () => {
    const { clock, room, server } = createEnvironment();
    const hostTransport = new FakeTransport();
    const guestTransport = new FakeTransport();

    server.connectClient({ roomId: room.room.roomId, token: 'host-token', connectionId: 'conn-host', transport: hostTransport });
    server.connectClient({ roomId: room.room.roomId, token: 'guest-token', connectionId: 'conn-guest', transport: guestTransport });

    clock.advance(45_001);
    const sweep = server.sweepBattleTimeouts();

    assert.equal(sweep.processedBattles, 1);
    assert.equal(hostTransport.messages.some((message) => message.type === 'battle.turn_resolved'), true);
    assert.equal(guestTransport.messages.some((message) => message.type === 'battle.turn_resolved'), true);

    const session = server.getBattleSession(room.room.roomId);
    assert.equal(session?.turn, 2);
    const debugView = server.getBattleDebugView(room.room.roomId);
    assert.equal(debugView?.timeouts.host.consecutive, 1);
    assert.equal(debugView?.timeouts.guest.consecutive, 1);
    assert.equal(debugView?.commands.filter((entry) => entry.source === 'timeout_auto').length, 2);
  });

  it('연속 timeout이 누적되면 timeout_forfeit로 종료한다', () => {
    const { clock, room, server } = createEnvironment();
    const hostTransport = new FakeTransport();
    const guestTransport = new FakeTransport();
    ...
    server.receiveMessage(
      'conn-host',
      buildChooseMoveCommand({ roomId: room.room.roomId, battleId, clientCommandId: 'host-cmd-1', turn: 1, moveSlot: 1 }),
    );
    clock.advance(45_001);
    server.sweepBattleTimeouts();

    server.receiveMessage(
      'conn-host',
      buildChooseMoveCommand({ roomId: room.room.roomId, battleId, clientCommandId: 'host-cmd-2', turn: 2, moveSlot: 1 }),
    );
    clock.advance(45_001);
    server.sweepBattleTimeouts();

    const ended = hostTransport.messages.findLast((message) => message.type === 'battle.ended');
    assert.ok(ended && ended.type === 'battle.ended');
    assert.equal(ended.payload.reason, 'timeout_forfeit');
    const session = server.getBattleSession(room.room.roomId);
    assert.equal(session?.phase, 'finished');
    assert.equal(session?.result?.loserSeat, 'guest');

    const debugView = server.getBattleDebugView(room.room.roomId);
    assert.equal(debugView?.timeouts.guest.consecutive, 2);
  });

  it('deadline이 지난 뒤 도착한 클라이언트 명령은 timeout rejection으로 거부한다', () => {
    const { clock, room, server } = createEnvironment();
    const hostTransport = new FakeTransport();
    const guestTransport = new FakeTransport();
    ...
    server.receiveMessage(
      'conn-guest',
      buildChooseMoveCommand({ roomId: room.room.roomId, battleId, clientCommandId: 'guest-late', turn: 1, moveSlot: 1 }),
    );

    const rejected = guestTransport.messages.findLast((message) => message.type === 'battle.command_rejected');
    assert.ok(rejected && rejected.type === 'battle.command_rejected');
    assert.equal(rejected.payload.code, 'PVP_COMMAND_TIMEOUT');
  });
});
