...
- `open` → go to **Step 1a** (open flow). If the **second** token is `lan`, run in LAN mode — bind `0.0.0.0` and advertise the machine's LAN IP instead of `127.0.0.1`.
...
- Default (no second token, or anything besides `lan`): **loopback mode** — bind `127.0.0.1`, advertise `127.0.0.1:<port>`. Only works when host and guest are on the **same machine**.
- Second token `lan`: **LAN mode** — bind `0.0.0.0`, advertise the machine's detected LAN IP. Works for two players on the **same local network** (e.g. same WiFi). The host's firewall must allow inbound TCP on the chosen port.
...
# LAN mode flips the bind address; in loopback mode LISTEN_HOST stays 127.0.0.1.
LISTEN_HOST=127.0.0.1  # set to 0.0.0.0 when the second token is `lan`
...
**In LAN mode, resolve the advertised host IP** (pick the first non-loopback IPv4 on the machine):
...
Use `ADVERTISED_HOST=$LAN_IP` in LAN mode, `ADVERTISED_HOST=127.0.0.1` in loopback mode. Use this value — NOT `127.0.0.1` — in the share strings below.

Tell the user:
- "세션 코드: `<SESSION_CODE>`"
- "호스트 주소: `<ADVERTISED_HOST>:<PORT>`"
- "상대방에게 위 코드와 주소를 공유하고, 상대방이 `/tkm:friendly-battle join <code>@<ADVERTISED_HOST>:<port>` 를 실행하도록 안내하세요."
- **In LAN mode only**, add: "⚠️ LAN 모드는 호스트 머신의 방화벽이 해당 TCP 포트(`<PORT>`)의 인바운드 연결을 허용해야 합니다. WSL2 호스트의 경우 Windows 방화벽도 확인하세요. 같은 WiFi/LAN 에 있는 상대방만 접속 가능합니다 (인터넷 경유 X)."
...
```
/tkm:friendly-battle open                       — 방을 열고 게스트를 기다림 (loopback, 같은 머신 전용)
/tkm:friendly-battle open lan                   — LAN 모드로 방 열기 (같은 네트워크의 다른 머신에서 접속 가능)
...
LAN 모드는 호스트 방화벽이 해당 포트의 인바운드 연결을 허용해야 합니다.
