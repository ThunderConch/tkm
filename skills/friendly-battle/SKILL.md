...
- `open` → go to **Step 1a** (open flow). Optional second token: a mode flag.
  - **Mode flag aliases** (parse short OR long form, both equivalent):
    - `-h` or `heuristic` → host-side daemon auto-pick (greedy heuristic)
    - `-a` or `ai` → host-side Claude auto-pick (skill-layer)
    - `-l` or `local` → loopback listen mode (`127.0.0.1`); player mode stays `manual`
    - omitted → LAN listen mode + `manual` player mode (default)
  - Examples (both forms accepted):
    - `/tkm:friendly-battle open` → LAN + manual
    - `/tkm:friendly-battle open -h` or `open heuristic` → LAN + heuristic
    - `/tkm:friendly-battle open -a` or `open ai` → LAN + ai
    - `/tkm:friendly-battle open -l` or `open local` → loopback + manual
- `join` → go to **Step 1b** (join flow). Optional mode flag (same aliases as `open`) BEFORE the `<code>@<host>:<port>` arg.
...
**Token normalization**: when you see `-h`/`-a`/`-l` in `$ARGUMENTS`, normalize internally to `heuristic`/`ai`/`local` BEFORE setting `$PLAYER_MODE` / `$LISTEN_HOST` in the bash blocks below. The CLI flag passed to `--player-mode` is always the long form.
...
- **`ai`**: You (Claude) pick the action inline. **Do NOT call AskUserQuestion.**
  ...
  **NO WORLD-KNOWLEDGE RULE** (type effectiveness): you MUST NOT infer type effectiveness from move/species names using your pre-trained Pokemon knowledge. Every `liveState.host.active.moves[i]` (or `liveState.guest.active.moves[i]` for the guest side) carries a **pre-computed** `typeEffectivenessVsOpponentActive` field — a number like `0`, `0.25`, `0.5`, `1`, `2`, or `4` — computed by the host's authoritative type_chart against the current opponent active pokemon's `types`. You MUST read that field directly and compare numerically. Do not multiply type matchups yourself, do not guess dual-type products, do not remember whether "용 vs 강철" is 0.5× — ALL of that is already baked into the number. If the field is missing (legacy event), fall back to assuming 1.0 and note it in your reasoning.
  For YOUR OWN side, use `liveState[yourRole]` + `moveOptions` + `partyOptions`. Build an explicit prompt from these fields, require the reply to be exactly one token (`move:N`, `switch:N`, or `surrender`), parse it, then call the `--action` subcommand with the parsed token. Retry once on malformed output; if parsing still fails, fall back to `move:1`.
...
```
/tkm:friendly-battle open                          — LAN 모드 + manual (기본)
/tkm:friendly-battle open -l   (or  open local)    — loopback 모드, manual (같은 머신 테스트)
/tkm:friendly-battle open -h   (or  open heuristic) — LAN + 호스트 daemon 자동 (greedy heuristic)
/tkm:friendly-battle open -a   (or  open ai)       — LAN + 호스트 Claude 자동 (skill-layer)
/tkm:friendly-battle join <code>@<host>:<port>     — 게스트로 참가, manual
/tkm:friendly-battle join -a <code>@<host>:<port>  — 게스트 Claude 자동 (-a 또는 ai)
/tkm:friendly-battle join -h <code>@<host>:<port>  — 게스트 heuristic (PR48 에선 no-op)
