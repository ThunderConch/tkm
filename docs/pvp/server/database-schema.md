...
관련 문서: [서버 데이터 모델](./data-model.md), [서버 아키텍처](./architecture.md), [API 계약](./api-contract.md), [치트 대응 정책](../security/anti-cheat.md), [PvP 작업 분해 / TODO](../implementation/todo-breakdown.md)
...
| `status` | `text` | not null | `waiting`, `ready`, `in_progress`, `finished`, `cancelled` |
| `current_phase` | `text` | not null | `waiting_for_players`, `awaiting_actions`, `resolving_turn`, `awaiting_replacement`, `finished` |
